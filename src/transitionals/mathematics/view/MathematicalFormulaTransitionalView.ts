...
export class MathematicalFormulaTransitionalView extends TransitionalView {
    static readonly transitionalName = "mathematics";
    ...
    constructor(contentNode: HTMLElement, metadata: TransitionalMetadata, context: TransitionalViewContext) {
        ...
        this.mathCodeRegionHoverNotificationThrottler = new TaskThrottler(
            MathematicalFormulaTransitionalView.MIN_TIME_BETWEEN_MATH_REGION_HOVER_NOTIFICATION
        );
    ...
    get mathCodeSplitByHoveredRegion(): MathCodeSplitByRange {
        return MathematicalFormulaTransitionalView.splitMathCodeByRange(
    ...
    private getDeepestMathRegionNodeContainingPosition(viewportX: number, viewportY: number): HTMLElement | null {
        ...
        for (let node of allNodesContainingPosition) {
            const codeRange = MathematicalFormulaTransitionalView.getRawMathRegionCodeRangeOfNode(node as HTMLElement);
            if (codeRange) {
